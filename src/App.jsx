...
export default function App(){
  ...
  useEffect(function(){
    if(bsQuery.data)setSt(fromBootstrap(bsQuery.data));
  },[bsQuery.data]);
  useEffect(function(){
    if(bsQuery.error&&bsQuery.error.status===401){setTok(null);mauth({token:null,user:null});qc.removeQueries({queryKey:['bootstrap']});}
  },[bsQuery.error]);
  useEffect(function(){
  ...
  // Single useMutation for all API writes — mutate(vars, {onSuccess, onError, onSettled})
  var apiMut=useMutation({
    mutationFn:function(req){return apiFetch(req.path,req.opts||{});},
    onError:function(err,req){if(req.onError)req.onError(err);else showToast(err.message);},
    onSuccess:function(data,req){if(req.onSuccess)req.onSuccess(data);},
    onSettled:function(data,err,req){if(req.onSettled)req.onSettled(data,err);},
  });
  function api(path,opts,cbs){apiMut.mutate({path:path,opts:opts,onSuccess:cbs&&cbs.ok,onError:cbs&&cbs.err,onSettled:cbs&&cbs.done});}
  ...
  function applyPct(hid,pct,tgt){
    ...
    if(!isTemp(hid)){api('/habits/'+hid+'/logs',{method:'POST',body:JSON.stringify({date:tod,count:count})},{err:function(e){showToast(e.message);}});}
  }
  function saveH(h){
    ...
    if(clean.id&&!isTemp(clean.id)){
      ...
      mu({editH:null});
      api('/habits/'+clean.id,{method:'PATCH',body:JSON.stringify(body)},{err:function(e){setSt(function(s){return Object.assign({},s,{habits:s.habits.map(function(x){return x.id===clean.id?prev:x;})});});showToast(e.message);}});
    } else {
      var tmpId='tmp_'+Date.now();
      setSt(function(s){return Object.assign({},s,{habits:s.habits.concat([Object.assign({},clean,{id:tmpId})])});});
      mu({editH:null});
      api('/habits',{method:'POST',body:JSON.stringify(body)},{ok:function(created){setSt(function(s){return Object.assign({},s,{habits:s.habits.map(function(x){return x.id===tmpId?created:x;})});});},err:function(e){setSt(function(s){return Object.assign({},s,{habits:s.habits.filter(function(x){return x.id!==tmpId;})});});showToast(e.message);}});
    }
  }
  function delH(id){
    ...
    if(!isTemp(id)){api('/habits/'+id,{method:'DELETE'},{err:function(e){setSt(function(s){return Object.assign({},s,{habits:s.habits.concat([prev])});});showToast(e.message);}});}
  }
  function addTask(text,tag){
    ...
    mu({taskInput:''});
    api('/tasks',{method:'POST',body:JSON.stringify({text:text.trim(),done:false,pinned:false,importance:3,urgency:3,list:'plan',tagId:tagVal?String(tagVal):null})},{ok:function(created){setSt(function(s){return Object.assign({},s,{tasks:s.tasks.map(function(tk){return tk.id===tmpId?Object.assign({},created,{tag:created.tagId}):tk;})});});},err:function(e){setSt(function(s){return Object.assign({},s,{tasks:s.tasks.filter(function(tk){return tk.id!==tmpId;})});});showToast(e.message);}});
  }
  function addTag(name){
    ...
    api('/tags',{method:'POST',body:JSON.stringify({name:name.trim()})},{ok:function(created){setSt(function(s){return Object.assign({},s,{tags:s.tags.map(function(tg){return tg.id===tmpId?created:tg;})});});},err:function(e){setSt(function(s){return Object.assign({},s,{tags:s.tags.filter(function(tg){return tg.id!==tmpId;})});});showToast(e.message);}});
  }
  function deleteTag(tagId){
    ...
    if(!isTemp(tagId)){api('/tags/'+tagId,{method:'DELETE'},{err:function(e){setSt(function(s){return Object.assign({},s,{tags:s.tags.concat([prev]),tasks:prevTasks,laterTasks:prevLater});});showToast(e.message);}});}
  }
  function setTaskTag(id,tagId,list){
    ...
    if(!isTemp(id)){api('/tasks/'+id,{method:'PATCH',body:JSON.stringify({tagId:tagId?String(tagId):null})},{err:function(e){showToast(e.message);}});}
  }
  function toggleTaskDone(id,list){
    ...
    if(!isTemp(id)){api('/tasks/'+id,{method:'PATCH',body:JSON.stringify({done:newDone})},{err:function(e){setSt(function(s){var arr=s[key].map(function(tk){return tk.id===id?Object.assign({},tk,{done:!newDone}):tk;});var r=Object.assign({},s);r[key]=arr;return r;});showToast(e.message);}});}
  }
  ...
  function setTaskPri(id,imp,urg,list){
    ...
    if(!isTemp(id)){api('/tasks/'+id,{method:'PATCH',body:JSON.stringify({importance:imp,urgency:urg})},{err:function(e){showToast(e.message);}});}
  }
  ...
  function deleteTask(id,list){
    ...
    if(!isTemp(id)){api('/tasks/'+id,{method:'DELETE'},{err:function(e){setSt(function(s){var r=Object.assign({},s);r[key]=s[key].concat([prev]);return r;});showToast(e.message);}});}
  }
  function updateTaskText(id,text,list){
    ...
    if(!isTemp(id)){api('/tasks/'+id,{method:'PATCH',body:JSON.stringify({text:text.trim()})},{err:function(e){showToast(e.message);}});}
  }
  function saveTaskEdit(id,updates,list){
    ...
    if(!isTemp(id)){var p=Object.assign({},u);if(p.tag!==undefined){p.tagId=p.tag?String(p.tag):null;delete p.tag;}api('/tasks/'+id,{method:'PATCH',body:JSON.stringify(p)},{err:function(e){setSt(function(s){var arr=s[key].map(function(tk){return tk.id===id?prev:tk;});var r=Object.assign({},s);r[key]=arr;return r;});showToast(e.message);}});}
  }
  ...
  function setSteps(v){
    ...
    api('/metrics',{method:'POST',body:JSON.stringify({date:tod,steps:next})},{err:function(e){showToast(e.message);}});
  }
  ...
  function editDayEntry(date,idx,text){
    ...
    if(entry&&entry._id){if(!text||!text.trim()){api('/journal/'+entry._id,{method:'DELETE'},{err:function(e){showToast(e.message);}});}else{api('/journal/'+entry._id,{method:'PATCH',body:JSON.stringify({text:text.trim()})},{err:function(e){showToast(e.message);}});}}
  }
  ...
  function addW(d){
    ...
    api('/metrics',{method:'POST',body:JSON.stringify({date:tod,water:next})},{err:function(e){showToast(e.message);}});
  }
  function addCal(v){
    ...
    api('/metrics',{method:'POST',body:JSON.stringify({date:tod,calories:next})},{err:function(e){showToast(e.message);}});
  }
  ...
  function setSleep(v){
    ...
    if(val){api('/metrics',{method:'POST',body:JSON.stringify({date:tod,sleep:parseFloat(val)})},{err:function(e){showToast(e.message);}});}
  }
  function saveGoals(key,draft){
    ...
    api('/goals',{method:'PUT',body:JSON.stringify(p)},{err:function(e){setSt(function(s){var g=Object.assign({},s.goals);g[key]=prev;return Object.assign({},s,{goals:g});});showToast(e.message);}});
  }
  function applyCal(r){
    var wz=ui.wiz;
