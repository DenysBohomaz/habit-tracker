...
var I18N = {
  en: {
    hello:"Hello, Champion",today:"Today",calendar:"Calendar",analytics:"Analytics",settings:"Settings",
    ...
    general:"General",week:"This Week",month:"This Month",allTime:"All Time",
  ...
  uk: {
    ...
    general:"Загальне",week:"Цей тиждень",month:"Цей місяць",allTime:"За весь час",
...
function initUi(){return{tab:"today",aTab:"general",yr:new Date().getFullYear(),mo:new Date().getMonth(),pDay:null,editH:null,detH:null,detFrom:null,form:Object.assign({},BLANK),thumb:{},confetti:false,hPopup:null,calPop:false,calIn:"",calMode:"+",sPanel:null,gDraft:null,wiz:null};}
...
export default function App(){
  ...
  var CN=st.goals.calories.norm,CX=st.goals.calories.max;
  ...
  function applyPct(hid,pct,tgt){
    var count=Math.round(pct/100*tgt),tod=TODAY;
    setSt(function(s){
  ...
  function applyCal(r){setSt(function(s){var g=Object.assign({},s.goals);g.calories={min:r.min,norm:r.norm,max:r.max};return Object.assign({},s,{goals:g});});mu({wiz:null});}
  ...
  function Sheet(p){return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:p.z||999}} onClick={p.onClose}>
      <div style={{background:card,borderRadius:"20px 20px 0 0",width:"100%",maxWidth:520,padding:"20px",boxShadow:"0 -4px 30px rgba(0,0,0,0.2)",maxHeight:"90vh",overflowY:"auto"}} onClick={function(e){e.stopPropagation();}}>
    ...
    <div style={{minHeight:"100vh",background:bg,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <div style={{background:card,borderBottom:"1px solid "+border,padding:"14px 20px 0",position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:520,margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:10}}>
            <p style={{fontWeight:700,fontSize:18,color:textMain,margin:"0 0 2px"}}>{t.hello} 👊</p>
            <p style={{fontSize:12,color:textSub,margin:0}}>{dLabel}</p>
          </div>
          <div style={{display:"flex",borderBottom:"1px solid "+border,margin:"0 -20px"}}>
            {["today","calendar","analytics","settings"].map(function(tab,i){
              var lbl=tab==="today"?t.today:tab==="calendar"?t.calendar:tab==="analytics"?t.analytics:t.settings;
      ...
      <div style={{maxWidth:520,margin:"0 auto",padding:"16px 16px 80px"}}>
          ...
          <div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:14}}>
              <Card s={{padding:"12px 8px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
                <p style={{fontSize:10,fontWeight:700,color:textSub,margin:"0 0 4px",textTransform:"uppercase",letterSpacing:"0.06em"}}>{t.sleep}</p>
                <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end"}}>
                  <SleepSVG hours={sh}/>
                </div>
                <div style={{width:"100%",marginTop:6,background:darkBg,borderRadius:8,border:"1px solid "+sCol,overflow:"hidden"}}>
                  <input type="number" min="0" max={st.goals.sleep.max} step="0.5" placeholder="-" value={sh} onChange={function(e){setSleep(e.target.value);}} style={{width:"100%",background:"transparent",border:"none",outline:"none",fontSize:18,fontWeight:700,color:sCol,textAlign:"center",padding:"5px 4px",fontFamily:"inherit"}}/>
                </div>
                <p style={{fontSize:10,color:sCol,margin:"4px 0 0",fontWeight:500}}>{sLbl}</p>
              </Card>

              <Card s={{padding:"12px 8px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
                <p style={{fontSize:10,fontWeight:700,color:textSub,margin:"0 0 4px",textTransform:"uppercase",letterSpacing:"0.06em"}}>{t.water}</p>
                <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end"}}>
                  <svg viewBox="0 0 80 160" width="44" height="88">
                    <defs><clipPath id="bc"><path d="M15 30 Q11 40 11 55 L11 138 Q11 149 20 149 L60 149 Q69 149 69 138 L69 55 Q69 40 65 30 Z"/></clipPath></defs>
                    <rect x="28" y="0" width="24" height="18" rx="4" fill="none" stroke="#bfdbfe" strokeWidth="2.5"/>
                    <path d="M14 30 Q10 40 10 55 L10 138 Q10 150 20 150 L60 150 Q70 150 70 138 L70 55 Q70 40 66 30 L28 18 L52 18 Z" fill="none" stroke="#bfdbfe" strokeWidth="2.5"/>
                    <rect x="26" y="-2" width="28" height="10" rx="3" fill="#60a5fa"/>
                    <rect x="0" y={fy} width="80" height={149-fy+10} clipPath="url(#bc)" fill={wCol} opacity="0.9"/>
                    {wPct>0&&<path d={"M10 "+fy+" Q25 "+(fy-5)+" 40 "+fy+" Q55 "+(fy+5)+" 70 "+fy} fill="none" stroke={wCol} strokeWidth="2" clipPath="url(#bc)" opacity="0.5"/>}
                    <text x="40" y="100" textAnchor="middle" fontSize="13" fontWeight="700" fill={wPct>0.5?"#fff":"#1d4ed8"}>{wl.toFixed(1)}L</text>
                    <text x="40" y="116" textAnchor="middle" fontSize="9" fill={wPct>0.5?"rgba(255,255,255,0.8)":"#3b82f6"}>of {WG}L</text>
                  </svg>
                </div>
              ...
              <Card s={{padding:"12px 8px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center"}}>
                <p style={{fontSize:10,fontWeight:700,color:textSub,margin:"0 0 4px",textTransform:"uppercase",letterSpacing:"0.06em"}}>{t.calories}</p>
                <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end"}}>
                  <svg viewBox="0 0 100 72" width="80" height="60" style={{marginTop:4}}>
                ...
                <p style={{fontSize:20,fontWeight:800,color:cCol,margin:"4px 0 0",lineHeight:1}}>{calT}</p>
                <p style={{fontSize:10,color:textSub,margin:"1px 0 4px"}}>kcal / {CN} {t.norm}</p>
                <PBar pct={Math.min(calT/Math.max(1,CN)*100,100)} col={cCol} h={4}/>
              ...
              <div key={g.time} style={{marginBottom:20}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {g.habits.map(function(h){
                    ...
                    return(
                      <div key={h.id} onClick={function(){mu({hPopup:{id:h.id,pct:getPct(h,TODAY,st.counts),target:h.target||1}});}} style={{display:"flex",alignItems:"center",gap:12,background:sc?sc.bg:card,border:"1px solid "+(sc?sc.bo:border),borderRadius:"12px",padding:"13px 14px",cursor:"pointer",boxShadow:"0 1px 2px rgba(0,0,0,0.04)"}}>
                        <div style={{width:22,height:22,borderRadius:6,border:"2px solid "+(sc?sc.tx:border),background:sc?sc.bg:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                          ...
                          {isPart&&<span style={{fontSize:8,fontWeight:800,color:sc.tx}}>{pct}%</span>}
                        </div>
                        ...
                        {h.target>1&&<span style={{fontSize:12,color:sc?sc.tx:textSub,fontWeight:600,background:sc?sc.bg:darkBg,padding:"2px 8px",borderRadius:20,border:"1px solid "+(sc?sc.bo:border)}}>{cnt}/{h.target}</span>}
            ...
            <Card>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:3}}>
                ...
                {Array.from({length:DIM},function(_,i){return i+1;}).map(function(d){
                  var key=cKey(d),st2=daySt(st.habits,key,st.counts),sc2=dayScore(st.habits,key,st.counts);
                  var isT=isCurMo&&d===tObj.getDate(),isFut=isCurMo&&d>tObj.getDate();
                  var face=(!isFut&&sc2!==null)?(sc2>=90?"😊":sc2>=60?"😐":"😢"):null;
                  return(
            ...
            {ui.pDay&&(function(){
              ...
              function HR(rp){var p=getPct(rp.h,key,st.counts);return(
                <div style={{padding:"8px 0",borderBottom:"1px solid "+border}}>
                  ...
                  {rp.h.notes&&<p style={{fontSize:12,color:textSub,margin:"4px 0 0 26px",lineHeight:1.5,background:darkBg,borderRadius:6,padding:"4px 8px"}}>{rp.h.notes}</p>}
                </div>
              );}
              return(
                <Sheet onClose={function(){mu({pDay:null});}}>
                  ...
                  {dn&&<div style={{background:darkBg,borderRadius:10,padding:"10px 14px",marginBottom:14,borderLeft:"3px solid "+border}}><p style={{fontSize:11,color:textLight,margin:"0 0 3px",fontWeight:600,textTransform:"uppercase"}}>Note</p><p style={{fontSize:13,color:textSub,margin:0,lineHeight:1.5,whiteSpace:"pre-wrap"}}>{dn}</p></div>}
        ...
        {ui.tab==="analytics"&&(function(){
          var tabs={
            general:{title:t.general,rows:[{icon:"📅",val:trk,label:t.daysTracked},{icon:"💪",val:st.habits.length,label:t.habits}],f:atF,p:atP,m:atM},
            week:{title:t.week,rows:[{icon:"📅",val:wkK.length,label:t.days},{icon:"🎯",val:wkK.length>0?Math.round(wkF/wkK.length*100)+"%":"0%",label:t.perfectRate}],f:wkF,p:wkP,m:wkM},
            month:{title:t.month,rows:[{icon:"📅",val:moK.length,label:t.days},{icon:"🎯",val:moK.length>0?Math.round(moF/moK.length*100)+"%":"0%",label:t.perfectRate}],f:moF,p:moP,m:moM},
            "all time":{title:t.allTime,rows:[{icon:"📅",val:atK.length,label:t.days},{icon:"🔥",val:bStr+"d",label:t.bestStreak}],f:atF,p:atP,m:atM}
          };
          var td=tabs[ui.aTab]||tabs["general"];
          return(
            <div>
              <div style={{display:"flex",gap:4,marginBottom:16,background:card,borderRadius:"12px",padding:4,boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
                {[["general",t.general],["week",t.week],["month",t.month],["all time",t.allTime]].map(function(pair){var a=ui.aTab===pair[0];return <div key={pair[0]} onClick={function(){mu({aTab:pair[0]});}} style={{flex:1,textAlign:"center",padding:"7px 4px",borderRadius:9,background:a?"#111827":"transparent",color:a?"#fff":textSub,fontSize:12,fontWeight:a?600:400,cursor:"pointer"}}>{pair[1]}</div>;})}
              </div>
              <p style={{fontSize:16,fontWeight:700,color:textMain,margin:"0 0 14px"}}>{td.title}</p>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:10}}>
                <Stat icon="✅" val={td.f} label={t.perfect} col={green} bg={greenBg} bo={greenBo}/>
                <Stat icon="🌓" val={td.p} label={t.partial} col={yellow} bg={yellowBg} bo={yellowBo}/>
                <Stat icon="❌" val={td.m} label={t.missed}  col={red}   bg={redBg}    bo={redBo}/>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>
                {td.rows.map(function(r){return <div key={r.label} style={{background:card,borderRadius:"12px",padding:"14px 8px",textAlign:"center",border:"1px solid "+border}}><p style={{fontSize:18,margin:"0 0 2px"}}>{r.icon}</p><p style={{fontSize:24,fontWeight:800,color:textMain,margin:"0 0 2px",lineHeight:1}}>{r.val}</p><p style={{fontSize:11,color:textSub,margin:0}}>{r.label}</p></div>;} )}
              </div>
            </div>
          );
        })()}
        ...
        {ui.tab==="settings"&&(
          <div>
            ...
            {(function(){var g=st.goals.calories;return(
              <Card s={{marginBottom:12,cursor:"pointer",padding:"16px 18px"}} onClick={function(){mu({wiz:{step:1,gender:null,W:"",H:"",A:"",goal:null,act:null,result:null}});}}>
                <div style={{display:"flex",alignItems:"center",gap:14}}>
                      ...
                      {[{f:"W",label:t.weight,unit:"kg",ph:"70"},{f:"H",label:t.height,unit:"cm",ph:"175"},{f:"A",label:t.age,unit:"years",ph:"25"}].map(function(fi){return(
                        <div key={fi.f} style={{marginBottom:16}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",border:"1.5px solid "+(wz[fi.f]?textMain:border),borderRadius:"8px",overflow:"hidden"}}>
                            <input type="number" min="0" placeholder={fi.ph} value={wz[fi.f]} onChange={function(e){var v=e.target.value,p=Object.assign({},wz);p[fi.f]=v;mu({wiz:p});}} style={{flex:1,fontSize:18,fontWeight:700,padding:"10px 14px",border:"none",outline:"none",color:textMain,background:"transparent",fontFamily:"inherit"}}/>
          ...
          return(
            <Sheet z={1000} onClose={function(){mu({hPopup:null});}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
                ...
                <div style={{flex:1}}><p style={{fontSize:16,fontWeight:700,margin:"0 0 2px",color:textMain}}>{h.name}</p><p style={{fontSize:12,color:textSub,margin:0}}>{t.howMuch}</p></div>
              ...
              <div style={{display:"flex",gap:10}}>
                <Btn onClick={function(){applyPct(h.id,pct,h.target||1);}} v="pri" s={{flex:1,fontSize:15,padding:"12px"}}>{t.save} — {pct}%</Btn>
